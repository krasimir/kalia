...
//   client = new LanguageClient("KaliaLS", "KaliaLS", serverOptions, clientOptions);
//   client.start();
//   client.onReady().then(() => {
//     clientReady = true;
...
//       client.sendNotification(EVENTS.GENERATE_ANALYSIS, {
//         uri,
...
//   const ignoreScopeNodes = ["WhileStatement", "Program"];
...
//       let postfix = "";
//       if (editor.selection.start.line >= node.start[0] - 1 && editor.selection.start.line <= node.end[0] - 1) {
//         postfix = " 👈";
//       }
//       if (isVariable(node)) {
...
function activate(context) {
  ...
  //     }
  //     if (clientReady) {
  //       client.sendNotification(EVENTS.NEW_SELECTION, {
  ...
  //   }
  // });
  // context.subscriptions.push(commands.registerCommand("Kalia.goto", gotoCommand));
